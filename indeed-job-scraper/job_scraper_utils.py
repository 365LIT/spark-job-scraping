...
def scrape_job_data(driver, country):
    df = pd.DataFrame({'Link': [''], 'Job Title': [''], 'Company': [''],
                       'Employer Active': [''], 'Location': [''], 'Description': ['']})
    job_count = 0
    # count = 0
    while True:
        # count += 1
        ...
        for i in boxes:
            ...
            if link_full:
                try:
                    # Navigate to the detailed job page
                    driver.get(link_full)
                    detail_soup = BeautifulSoup(driver.page_source, 'lxml')
                    description_div = detail_soup.find('div', {
                    ...
                    print(f"Error extracting description for {job_title}: {e}")
                finally:
